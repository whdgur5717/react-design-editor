...
const resolveCornerTokens = (corner: NormalizedCorner, tokenRefs: Map<string, TokenRef>): OutputNormalizedCorner => {
	const radius =
...
const resolveTextTokens = (
	...
	const runs = text.runs.map((run, index) => {
		...
		const paragraphSpacingAlias =
			getAlias(boundVariables, "paragraphSpacing") ?? getAliasFromArray(nodeBoundVariables?.["paragraphSpacing"], index)
		const paragraphIndentAlias =
			getAlias(boundVariables, "paragraphIndent") ?? getAliasFromArray(nodeBoundVariables?.["paragraphIndent"], index)
...
const buildLayoutGrids = (node: SceneNode, tokenRefs: Map<string, TokenRef>): OutputLayoutGrid[] | undefined => {
	...
	return layoutGrids.map((grid, index) => {
		...
		return {
			...
			count: count === undefined ? undefined : applyAliasToken(count, getAlias(boundVariables, "count"), tokenRefs),
			offset: offset === undefined ? undefined : applyAliasToken(offset, getAlias(boundVariables, "offset"), tokenRefs),
			gutterSize:
...
const buildInstanceRef = async (node: SceneNode): Promise<InstanceRef | undefined> => {
	...
	const variantProperties =
		"variantProperties" in node ? (node as { variantProperties?: Record<string, string> }).variantProperties : undefined
...
const enrichStyle = (
	...
	const effects: NormalizedValue<Array<TokenizedValue<NormalizedEffect>>> =
		normalizedStyle.effects.type === "uniform" ? { type: "uniform", value: effectsValue } : { type: "mixed", values: [] }

	const stroke = normalizedStyle.stroke ? resolveStrokeTokens(normalizedStyle.stroke, strokeAliases, tokenRefs) : null
	...
	const opacityValue = typeof extractedStyle.effects.opacity === "number" ? extractedStyle.effects.opacity : undefined
