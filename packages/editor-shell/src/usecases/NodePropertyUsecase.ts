...
import { SetInstancePropValuesCommand } from "../commands/node/SetInstancePropValuesCommand"
...
export class NodePropertyUsecase {
	...
	updatePropValues(nodeId: string, propValues: Record<string, unknown>) {
		const cmd = new SetInstancePropValuesCommand(this.receiver, nodeId, propValues, `prop-${nodeId}`)
		this.history.execute(cmd)
	}
}
